import sys
from collections import defaultdict
import re
# Розробіть Python-скрипт для аналізу файлів логів. Скрипт повинен вміти читати лог-файл, 
# переданий як аргумент командного рядка, і виводити статистику за рівнями логування наприклад, 
# INFO, ERROR, DEBUG. Також користувач може вказати рівень логування як другий аргумент 
# командного рядка, щоб отримати всі записи цього рівня.
 
def parse_log_line(line: str) -> dict:
    if not re.match(r'(\d{4}(?:-\d{2}){2} \d{2}(?::\d{2}){2}) (\w+) (.*)',line):   # Перевірка рядка на відповідність шаблону лог-рядка.
        return None
    date,time,level,*message = line.split(' ')
    return {'date':date,'time':time,'level':level,"message":' '.join(message)}

def load_logs(file_path: str) -> list:                                                      # Функція для завантаження лог-файлу і повернення списку записів.
    with open(file_path,"r+") as logfile:
        lines=list(filter(lambda x: x is not None,(map(parse_log_line,logfile.readlines()))))
    return lines

def filter_logs_by_level(logs: list, level: str) -> list:                               # Функція для фільтрації записів за вказаним рівнем логування.
    return list(filter(lambda x: x['level']==level,logs))

def count_logs_by_level(logs: list) -> dict:                                                # Функція для підрахунку кількості записів для кожного рівня логування.
    cntr=defaultdict()
    cntr.default_factory=int  
    for log in logs:
        cntr[log['level']]+=1
    return dict(cntr)

def display_log_counts(counts: dict):                                                       # Функція для виведення статистики за рівнями логування.
    print(f"{'Рівень логування ':20}| Кількість ")
    print(f"{'-'*20}|{'-'*10}")
    print("\n".join(list(map(lambda x:f"{x:20}| {counts[x]}",counts))))
    

if __name__=="__main__":
    filename=sys.argv[1] 
    logs=load_logs(file_path=filename)
    display_log_counts(count_logs_by_level(logs))
    if len(sys.argv)==3:
        level=sys.argv[2]
        logs=filter_logs_by_level(logs,level)
        print()
        print(f"Деталі логів для рівня '{level}':")
        print(''.join(map(lambda x:f"{x['date']} {x['time']} - {x['message']}",logs)))
    